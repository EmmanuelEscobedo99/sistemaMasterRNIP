...
const ConsultarErrores = () => {
  ...
  const handleLimpiarErrors = async (event) => {
    ...
    if (!mensaje.trim()) {
      // Agregar esta validación en el lugar adecuado
      Swal.fire({
        title: 'Error',
        ...
        try {
          ...
          for (const [formName, items] of Object.entries(erroresAgrupados)) {
            for (const item of items) {
              await rechazarRegistro(idAlterna, LLAVE, formName, item.nombre, mensaje, item.idBloqueFuncional); // Ahora se pasa el idBloqueFuncional
            }
          }
          ...
          Swal.fire({
            title: 'Error',
            text: 'Por favor ingrese un mensaje sobre los errores.',
            icon: 'error',
            confirmButtonText: 'Aceptar',
          });
        }
  ...
  const rechazarRegistro = async (ID_ALTERNA, LLAVE, FORMULARIO, CAMPO, DESCRIPCION, ID_BLOQUE_FUNCIONAL) => {
    console.log(ID_BLOQUE_FUNCIONAL)
    try {
      // Realizas la solicitud PUT aquí, pasando los datos necesarios
      await api.put(
        `rechazar/rechazarRegistro/${ID_ALTERNA}/${LLAVE}/${FORMULARIO}/${CAMPO}/${DESCRIPCION}/${ID_BLOQUE_FUNCIONAL}`
      );
    } catch (error) {
