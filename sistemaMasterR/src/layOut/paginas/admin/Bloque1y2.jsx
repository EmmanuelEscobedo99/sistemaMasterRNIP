...
const Bloque1y2 = () => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      await cargarInternosBloque1y2D();
      setResultados(internosBloque1y2D);

      // Asegura que la pantalla de carga dure al menos 2 segundos
      await new Promise((resolve) => setTimeout(resolve, 2000));
      setLoading(false); // Cambiar a false cuando los datos hayan cargado
    };

    cargarDatos();
  }, [internosBloque1y2D, cargarInternosBloque1y2D]);
