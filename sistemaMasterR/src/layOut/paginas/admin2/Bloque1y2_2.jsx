...
const Bloque1y2_2 = () => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        // Cargar los datos
        await cargarInternosBloque1y2();
        
        // Asegurar que la pantalla de carga dure al menos 2 segundos
        await new Promise((resolve) => setTimeout(resolve, 2000)); // Espera 2 segundos

        // Establecer los resultados cargados
        setResultados(internosBloque1y2);
      } catch (error) {
        console.error("Error al cargar los datos:", error);
      } finally {
        // Cambiar el estado de loading a false después de 2 segundos
        setLoading(false);
      }
    };

    cargarDatos();
  }, [internosBloque1y2, cargarInternosBloque1y2]);
