...
router.post("/upload", upload.any(), async (req, res) => {
  try {
    ...
    console.log("🛑 Datos en req.body:", JSON.stringify(req.body, null, 2));
    ...
    // 🔍 Validar que imagen1, imagen2 e imagen3 tengan los grupos correctos
    const requiredGroups = ["A", "B", "C"];
    for (let i = 0; i < 3; i++) {
      const imageKey = `imagen${i + 1}`;
      const grupoRecibido = req.body[`${imageKey}_grupo`];

      console.log(`🔎 Revisando ${imageKey}: grupo recibido -> ${grupoRecibido}, grupo esperado -> ${requiredGroups[i]}`);
    ...
    for (const file of req.files) {
      ...
      const grupo = req.body[`${file.fieldname}_grupo`] || "A"; // Obtener grupo desde FormData
